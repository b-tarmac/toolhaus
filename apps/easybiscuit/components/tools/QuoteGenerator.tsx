...
export default function QuoteGenerator(_props: EasyBiscuitToolProps) {
  const { user } = useAuth();
  const usage = useToolUsage("quote-generator");
  ...
  const handleGenerate = async () => {
    ...
    const data: InvoiceData = {
      ...
      invoiceNumber: quoteNumber || "QUOTE-001",
      issueDate: issueDate || today(),
      dueDate: validUntil || nextMonth(),
      currency,
    ...
    try {
      const pdf = await generateInvoicePdf(data, "quote");
